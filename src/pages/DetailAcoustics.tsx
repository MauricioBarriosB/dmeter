...
import { loadAcousticsHistory, type AcousticsRecord, isStorageValid } from "../helpers/analysisStorage";
...
export default function DetailAcoustics() {
    ...
    useEffect(() => {
        ...
        // Load config and history in parallel
        Promise.all([
            fetchFromApi<AcousticsConfig>("acousticsConfig"),
            loadAcousticsHistory()
        ])
            .then(([configData, history]) => {
                ...
                if (id) {
                    ...
                    if (found) {
                        setRecord(found);
                        setMetrics(calculateAcousticMetrics(found, configData));
                    } else {
                        setNotFound(true);
                    }
                } else {
    ...
    if (isValid === null || storageValid === null || !config) return <LoadingCard message="Loading..." maxWidth="7xl" />;
