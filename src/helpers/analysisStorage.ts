 ...
 * Shows toast on first failure
 */
export async function validateStorageAccess(): Promise<boolean> {
    const isValid = await validateStorageKey();
    if (!isValid && !toastShown) {
        toastShown = true;
        addToast({
            title: "Access Denied",
            description: "Invalid storage key. This application deployment is not authorized.",
            color: "danger",
            timeout: 8000,
        });
    }
...
/**
 * Get current validation state synchronously (after initial validation)
 */
