...
function computeAverageSpectrum(samples: Float32Array, fftSize: number, hopSize: number): Float32Array {
    ...
    if (windowCount > 0) {
        for (let i = 0; i < numBins; i++) {
            avgSpectrum[i] = avgSpectrum[i] > 0
                ? 20 * Math.log10(avgSpectrum[i] / windowCount)
                : -100;
        }
    }
...
function calculateSpectralSpread(frequencyData: Float32Array, sampleRate: number, fftSize: number, centroid: number): number {
    let weightedSum = 0;
    let magnitudeSum = 0;
    const binWidth = sampleRate / fftSize;
...
function calculateSpectralSkewness(frequencyData: Float32Array, sampleRate: number, fftSize: number, centroid: number, spread: number): number {
...
function calculateSpectralKurtosis(frequencyData: Float32Array, sampleRate: number, fftSize: number, centroid: number, spread: number): number {
    if (spread === 0) return 0;
    let weightedSum = 0;
    let magnitudeSum = 0;
    const binWidth = sampleRate / fftSize;
...
function calculateBandEnergy(frequencyData: Float32Array, fftSize: number, sampleRate: number, lowFreq: number, highFreq: number): number {
    const binWidth = sampleRate / fftSize;
...
function analyzeSilence(samples: Float32Array, sampleRate: number, threshold: number = 0.01): {
    percentage: number;
    leading: number;
    trailing: number;
} {
...
export async function analyzeAudioFile(file: File): Promise<{
    ...
    const spectralSkewness = calculateSpectralSkewness(frequencyData, sampleRate, fftSize, spectralCentroid, spectralSpread);
    const spectralKurtosis = calculateSpectralKurtosis(frequencyData, sampleRate, fftSize, spectralCentroid, spectralSpread);
    ...
    const fundamentalFrequency = estimateFundamentalFrequency(samples.slice(0, Math.min(sampleRate * 2, samples.length)), sampleRate);
...
export function getSpectralBalanceAssessment(octaveBands: { frequency: number; magnitude: number }[]): string {
    ...
    const mids = octaveBands.filter((b) => b.frequency > 250 && b.frequency <= 2000).reduce((sum, b) => sum + b.magnitude, 0);
