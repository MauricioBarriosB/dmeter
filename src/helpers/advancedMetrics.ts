...
export function calculateSpectralCentroid(
  ...
  const weightedSum = linearPeaks.reduce(
    (sum, p) => sum + p.frequency * p.amplitude,
    0
  );
...
export function calculateSpectralFlatness(
  ...
  // Geometric mean
  const logSum = linearAmplitudes.reduce(
    (sum, a) => sum + Math.log(a + 1e-10),
    0
  );
  ...
  const arithmeticMean =
    linearAmplitudes.reduce((sum, a) => sum + a, 0) / linearAmplitudes.length;
