...
export default function Materials() {
    ...
    useEffect(() => {
        ...
        fetchFromApi<MaterialsConfig>("materials-config")
            .then((data) => {
                setConfig(data);
                setIsLoading(false);
            })
    ...
    const handleSubmit = (e: React.SubmitEvent) => {
        ...
        const allMaterialsOptions = config.categories.flatMap((cat) => cat.options);
        ...
        const record: MaterialsReportRecord = {
            ...
            buildTypeLabel: config.buildTypes.find((opt) => opt.key === formData.buildType)?.label || "",
        ...
        clearHistory()
            .then(() => {
                addToast({
                    ...
                    description: "All materials reports have been deleted.",
                    color: "success",
                });
            })
            .catch((err) => {
    ...
    if (isLoading) {
        return <LoadingCard maxWidth="7xl" />;
    }
    ...
    return (
        <div className="max-w-7xl mx-auto">
            <div className="mb-10">
                ...
                <HowToUse
                    title="How to Use Materials Reports"
                    steps={[
                        {
                            number: 1,
                            title: "Enter Report Name",
                            description: "Give your materials report a descriptive name for easy identification.",
                        },
                        {
                            number: 2,
                            title: "Select Build Type",
                            description: "Choose the type of space you want to build from 12 professional options.",
                        },
                        {
                            number: 3,
                        ...
                        },
                        {
                            number: 4,
                            title: "Build Report",
                            description: "Click on button Generate Report to start and store the data.",
                        },
                        {
                            number: 5,
                            title: "View Results",
                ...
                <CardBody>
                    <form onSubmit={handleSubmit} className="flex flex-col gap-6">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                            {config.categories.slice(0, 4).map((category) => (
                                <Select
                                    key={category.key}
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                            {config.categories.slice(4, 8).map((category) => (
                                <Select
                                    key={category.key}
                                    label={category.label}
                                    placeholder={`Select ${category.label.toLowerCase()}`}
